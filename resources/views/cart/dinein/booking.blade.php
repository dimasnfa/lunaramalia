...
/* Custom SweetAlert2 styling untuk popup rekomendasi */
...
.recommendation-add-btn.btn-success {
...
.recommendation-add-btn .fa-spinner {
...
/* Price styling */
.recommendation-card .text-success {
    background: linear-gradient(45deg, #28a745, #20c997);
    -webkit-background-clip: text;
    -webkit-text-fill-color: transparent;
    background-clip: text;
...
/* Cart icon animations - ADDED BACK FROM OLD CODE */
.cart-badge-animate {
                        ...
                        <button class="nav-link {{ $index === 0 ? 'active' : '' }}" 
                            ...
                            @php
                                $iconMap = [
                                    'Makanan' => 'utensils',
                                    'Minuman' => 'coffee',
                                    'Nasi dan Mie' => 'drumstick-bite',
                                    'Aneka Snack' => 'cookie-bite'
                                ];
                                $icon = $iconMap[$kategori->nama_kategori] ?? 'list';
                            @endphp
...
<!-- ✅ Script utama untuk add to cart dengan popup rekomendasi menggunakan Algoritma Apriori -->
<script>
document.addEventListener('DOMContentLoaded', function() {
    const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
    
    // ✅ Function untuk update cart count di navbar - FIXED COMPATIBILITY
    function updateCartCountInNavbar(cartCount) {
        // Multiple selectors untuk kompatibilitas dengan berbagai ID/class
        ...
        // Cari elemen cart count yang ada
        ...
        if (cartCountElement) {
            ...
            if (currentCount !== cartCount) {
                ...
                // Show/hide badge berdasarkan count
                if (cartCount > 0) {
            ...
            console.warn('Cart count element not found with any of the selectors:', possibleSelectors);
        }
    }

    // ✅ Function untuk refresh cart count dari server
    function refreshCartCount() {
        fetch('/dinein/cart-count', {
            method: 'GET',
            headers: {
    ...
    // ✅ Function untuk menampilkan rekomendasi dari server dengan UI yang enhanced
    ...
    function addRecommendedToCart(menuId, menuName, menuPrice) {
        ...
        if (!jenisPesanan || (jenisPesanan === 'dinein' && !mejaId)) {
            Swal.fire({
                icon: 'warning',
                title: 'Data Tidak Lengkap',
                text: 'Silakan lengkapi data pemesanan terlebih dahulu',
                confirmButtonColor: '#f44336'
            });
            return;
        }
    ...
    // ✅ Enhanced add to cart dengan popup rekomendasi Apriori
...
window.addToCartDinein = function(menuId, menuName, menuPrice) {
